...
COMMAND="$PRIVATE_HOME/.nodebrew/current/bin/node $PRIVATE_HOME/.nodebrew/current/bin/yarn build-full-dev" && OUTPUT=`cd $ROOT && $COMMAND` && /usr/bin/php7.4 $ROOT/cron/commandLog.php "$COMMAND" "$OUTPUT"